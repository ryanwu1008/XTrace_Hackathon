...
test("terminal 0025 journals a local-only Deal-bound research registry without parallel XTrace", () => {
  assert.equal(existsSync(migrationPath), true);
  const plan = discoverMigrationPlan({
    directory: fileURLToPath(new URL("../../drizzle/", import.meta.url)),
    journalPath,
  });
  assert.equal(plan.terminal.index, 25);
  assert.equal(plan.terminal.tag, "0025_report_deal_universe_authority");
